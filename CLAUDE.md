...
A NestJS REST API for managing credit card purchases and monthly invoices (users, cards, purchases, invoices), with JWT authentication (Passport) and Prisma/PostgreSQL for persistence. Swagger docs are served at `/api-docs`.
...
The codebase is mid-migration from a flat Nest module layout to DDD / Clean Architecture. **Both styles currently coexist**, which is the main thing to know before touching auth or users code:
...
Prisma schema (`prisma/schema.prisma`) models: `User` → `Card` → `Purchase`/`Invoice`, plus `Category`. Migrations are in `prisma/migrations/`.
